...
class SM_Settings_Podcast extends SM_Settings_Page { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
	...
	public function get_settings() {
		$settings = apply_filters( 'sm_podcast_settings', array( // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
			...
			array(
				...
				'desc'        => __( 'This will display at the &ldquo;Artist&rdquo; in the iTunes Store.', 'sermon-manager-revival' ),
			),
			array(
			...
			array(
				...
				'desc'  => __( 'This JPG will serve as the Podcast artwork in the iTunes Store. The image must be between 1,400px by 1,400px and 3,000px by 3,000px or else iTunes will not accept your feed.', 'sermon-manager-revival' ),
			),
			array(
			...
			array(
				...
				'type'     => 'checkbox',
				'id'       => 'podtrac',
				'desc'     => __( 'Enables PodTrac tracking.', 'sermon-manager-revival' ),
				// translators: %s <a href="http://podtrac.com">podtrac.com</a>.
				'desc_tip' => wp_sprintf( __( 'For more info on PodTrac or to sign up for an account, visit %s', 'sermon-manager-revival' ), '<a href="http://podtrac.com">podtrac.com</a>' ),
				'default'  => 'no',
			),
			array(
				...
				'desc'     => __( 'Enables showing of HTML in iTunes description field. Uncheck if description looks messy.', 'sermon-manager-revival' ),
				'desc_tip' => __( 'It is recommended to leave it unchecked. Uncheck if the feed does not validate.', 'sermon-manager-revival' ),
				'default'  => 'no',
			),
			array(
				...
				'desc'     => __( 'Enables redirection of podcast from old to new URL.', 'sermon-manager-revival' ),
				'desc_tip' => __( 'You can use relative or absolute URLs.', 'sermon-manager-revival' ),
			),
			array(
			...
			array(
				'title'       => __( 'Number of podcasts to show', 'sermon-manager-revival' ),
				'type'        => 'number',
				'id'          => 'podcasts_per_page',
				'placeholder' => get_option( 'posts_per_rss' ),
			),
			array(
				'title'       => __( 'iTunes Podcast URL', 'sermon-manager-revival' ),
				'type'        => 'text',
			...
			array(
				'title'       => __( 'Android Podcast URL', 'sermon-manager-revival' ),
				'type'        => 'text',
			...
			array(
				'title'       => __( 'Overcast Podcast URL', 'sermon-manager-revival' ),
				'type'        => 'text',
	...
	public function after() {
		...
		<div>
			<p>
				<label for="feed_url"><?php echo esc_html__( 'Feed URL to Submit to iTunes', 'sermon-manager-revival' ); ?></label>
			...
			<p>
				...
				echo wp_sprintf( esc_html__( 'Use the %s to diagnose and fix any problems before submitting your Podcast to iTunes.', 'sermon-manager-revival' ), '<a href="' . esc_url( 'http://www.feedvalidator.org/check.cgi?url=' . site_url( '/' ) . SermonManager::getOption( 'archive_slug', 'sermons' ) . '/feed/' ) . '" target="_blank">' . esc_html__( 'Feed Validator', 'sermon-manager-revival' ) . '</a>' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				?>
			</p>
			<p>
				<?php
				// translators: %s see msgid "Submit Your Podcast".
				echo wp_sprintf( esc_html__( 'Once your Podcast Settings are complete and your Sermons are ready, it&rsquo;s time to %s to the iTunes Store!', 'sermon-manager-revival' ), '<a href="https://www.apple.com/itunes/podcasts/specs.html#submitting" target="_blank">' . esc_html__( 'Submit Your Podcast', 'sermon-manager-revival' ) . '</a>' );
				?>
			</p>
			<p>
				<?php
				// translators: %s see msgid "FeedBurner".
				echo wp_sprintf( esc_html__( 'Alternatively, if you want to track your Podcast subscribers, simply pass the Podcast Feed URL above through %s. FeedBurner will then give you a new URL to submit to iTunes instead.', 'sermon-manager-revival' ), '<a href="http://feedburner.google.com/" target="_blank">' . esc_html__( 'FeedBurner', 'sermon-manager-revival' ) . '</a>' );
				?>
			</p>
			<p>
				<?php
				// translators: %s see msgid "iTunes FAQ for Podcast Makers".
				echo wp_sprintf( esc_html__( 'Please read the %s for more information.', 'sermon-manager-revival' ), '<a href="https://www.apple.com/itunes/podcasts/creatorfaq.html" target="_blank">' . esc_html__( 'iTunes FAQ for Podcast Makers', 'sermon-manager-revival' ) . '</a>' );
				?>
			</p>
		</div>
		<?php
