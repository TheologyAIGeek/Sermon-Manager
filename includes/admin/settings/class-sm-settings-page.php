...
abstract class SM_Settings_Page { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
	...
	public function output_sections() {
		...
		foreach ( $sections as $id => $label ) {
			echo '<li><a href="' . esc_url( admin_url( 'edit.php?post_type=wpfc_sermon&page=sm-settings&tab=' . $this->id . '&section=' . sanitize_title( $id ) ) ) . '" class="' . ( $current_section == $id ? 'current' : '' ) . '">' . esc_html( $label ) . '</a> ' . ( end( $array_keys ) == $id ? '' : '|' ) . ' </li>';
		}
