...
class SM_Import_SM {
	...
	function XMLparse( $file ) {
		...
		if ( ! preg_match( '/^\d+\.\d+$/', $this->wxr_version ) ) {
			...
			return new WP_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'sermon-manager-revival' ) );
		}
	...
	function process_attachment( $post, $url ) {
		if ( ! $this->fetch_attachments ) {
			return new WP_Error( 'attachment_processing_error', __( 'Fetching attachments is not enabled', 'sermon-manager-revival' ) );
		}
		...
		if ( $info ) {
			...
			return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'sermon-manager-revival' ) );
		}
	...
	function backfill_parents() {
		...
		foreach ( $this->post_orphans as $child_id => $parent_id ) {
			...
			if ( $local_child_id && $local_parent_id ) {
				$wpdb->update( $wpdb->posts, array( 'post_parent' => $local_parent_id ), array( 'ID' => $local_child_id ), '%d', '%d' );  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Direct, uncacheable DB access for bulk import/export/install/upgrade routines.
			}
		}
	}
	...
	function backfill_attachment_urls() {
		...
		foreach ( $this->url_remap as $from_url => $to_url ) {
			...
			$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->posts} SET post_content = REPLACE(post_content, %s, %s)", $from_url, $to_url ) );  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Direct, uncacheable DB access for bulk import/export/install/upgrade routines.
			// remap enclosure urls.
			$result = $wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->postmeta} SET meta_value = REPLACE(meta_value, %s, %s) WHERE meta_key='enclosure'", $from_url, $to_url ) );  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Direct, uncacheable DB access for bulk import/export/install/upgrade routines.
		}
	}
