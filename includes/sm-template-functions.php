...
/**
 * Returns the player engine to use for video and uploaded-file audio playback.
 *
 ...
 * engine (Plyr, Mediaelement, etc.) leaves the default as an uploaded file.
 *
...
/**
 * Returns the stored audio URL for a sermon for a specific source type.
 *
 * @param int    $post_id Sermon post ID.
 * @param string $type    'file', 'spotify', or 'apple'.
 *
 * @return string URL, or empty string if none stored.
 *
 * @since 2.16.0
 */
function wpfc_get_audio_url_for_type( $post_id, $type ) {
	switch ( $type ) {
		case 'spotify':
			return (string) get_post_meta( $post_id, 'sermon_audio_spotify', true );
		case 'apple':
			return (string) get_post_meta( $post_id, 'sermon_audio_apple', true );
		default:
 ...
 * the chosen source has no URL, it falls back to an uploaded file when present.
 *
 * @param int|WP_Post|null $post Sermon ID or object. Defaults to the current post.
 *
 * @return array {
 *     @type string $type 'file', 'spotify', or 'apple'.
 *     @type string $url  The resolved audio URL (empty string if none set).
 * }
 *
 * @since 2.16.0
 */
function wpfc_get_sermon_audio_source( $post = null ) {
	...
	if ( ! $post_id ) {
		return array( 'type' => 'file', 'url' => '' );
	}

	$selected = get_post_meta( $post_id, 'sermon_audio_source', true );

	// An empty or "default" selection follows the site-wide default source.
	...
	// If the chosen source has no URL but an uploaded file exists, fall back to it
	// so a player still renders instead of silently disappearing.
	if ( '' === $url && 'file' !== $type ) {
 ...
 *
 * @param string $url The share URL pasted into the sermon settings.
 *
 * @return string|false Embed iframe HTML, or false if the URL is not a
...
function wpfc_get_audio_embed_html( $url ) {
	...
	// Spotify (episode, show, track, album, or playlist).
	if ( preg_match( '#open\.spotify\.com/(?:embed/)?(episode|show|track|album|playlist)/([A-Za-z0-9]+)#i', $url, $m ) ) {
		$src    = 'https://open.spotify.com/embed/' . strtolower( $m[1] ) . '/' . $m[2];
		$height = ( 'show' === strtolower( $m[1] ) ) ? 232 : 152;
		 ...
		 * @param string $url    Original URL pasted by the user.
		 *
		 * @since 2.16.0
		 */
	...
	// Apple Podcasts.
	if ( preg_match( '#(?:embed\.)?podcasts\.apple\.com/(\S+)#i', $url, $m ) ) {
		$src = 'https://embed.podcasts.apple.com/' . $m[1];
 ...
 * @since 2.16.0 Renders Spotify/Apple Podcasts embeds when the URL is one.
...
function wpfc_render_audio( $source = '', $seek = null ) {
	...
	if ( is_numeric( $source ) ) {
		...
		switch ( $object->post_type ) {
			case 'wpfc_sermon':
				$audio  = wpfc_get_sermon_audio_source( $object );
				$source = $audio['url'];
				break;
			case 'attachment':
	...
	// Render a Spotify or Apple Podcasts embed if the source is one of those.
	...
	// Get the current player engine (Spotify/Apple global defaults fall back to Plyr).
	$player = wpfc_get_player_engine();
