...
class SermonManager { // phpcs:ignore
	...
	public static function maybe_print_cloudflare_plyr() {
		...
		foreach ( $GLOBALS['sm_plyr_scripts'] as $script ) {
			echo '<script type="text/javascript" data-cfasync="false" src="' . esc_url( $script->src ) . '"></script>'; // phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript -- Prints already-registered local Plyr scripts with a Cloudflare Rocket Loader opt-out attribute; only runs when the admin enables the Cloudflare workaround option.
	...
	public static function register_scripts_styles() {
		wp_register_script( 'wpfc-sm-fb-player', SM_URL . 'assets/vendor/js/facebook-video.js', array(), SM_VERSION, true );
		...
		wp_register_script( 'wpfc-sm-plyr-loader', SM_URL . 'assets/js/plyr' . ( ( defined( 'WP_DEBUG' ) && WP_DEBUG === true ) ? '' : '.min' ) . '.js', array( 'wpfc-sm-plyr' ), SM_VERSION, true );
		wp_register_script( 'wpfc-sm-verse-script', SM_URL . 'assets/vendor/js/verse.js', array(), SM_VERSION, true );
